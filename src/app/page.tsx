...
export default function LoginPage() {
  ...
  const confirmLogin = async () => {
    // For prototype, any provider button goes to home
    router.push('/home');
  };
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-brand-softwhite">
        ...
        <div className="absolute bottom-12 left-12 right-12">
          <div className="backdrop-blur-md bg-white/10 border border-white/20 p-8 rounded-2xl shadow-large">
            <h2 className="font-display text-4xl text-brand-snow mb-4 leading-tight drop-shadow-md">
              Timeless elegance,<br />redefined.
            </h2>
            <p className="font-sans text-brand-snow/90 text-sm tracking-wide">
              Discover the latest premium collections crafted for your everyday style.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 h-screen flex flex-col justify-center px-8 sm:px-16 lg:px-32 relative overflow-y-auto">
        ...
        >
          <div className="text-center mb-10">
            <h1 className="font-display text-4xl text-brand-midnight mb-3 tracking-tight">Welcome to Nikky's</h1>
            <p className="text-brand-graphite text-sm">Please sign in to continue to the premium store.</p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleCredentialsLogin}>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <Link href="#" className="text-xs font-medium text-brand-champagne hover:text-brand-softgold transition-colors">Forgot password?</Link>
              </div>
              <input 
                type="password" 
                id="password" 
          ...
          <div className="mt-12 text-center text-sm text-brand-graphite">
            Don't have an account? <Link href="/signup" className="font-medium text-brand-midnight border-b border-brand-midnight pb-0.5 hover:text-brand-champagne hover:border-brand-champagne transition-colors">Create one</Link>
          </div>
        </motion.div>
      </div>
      ...
      {providerToConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-charcoal/40 backdrop-blur-sm">
          ...
          >
            <h3 className="font-display text-2xl text-brand-midnight mb-2">Confirm Login</h3>
            <p className="text-brand-graphite text-sm mb-6">
              You are about to sign in using your {providerToConfirm} account. Do you want to continue?
            </p>
